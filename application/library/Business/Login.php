<?php
namespace Business;
class Login  extends BusinessAbstract {
    use Instance;
...
//    protected $_savetime = 2592000;
    ...
    public function getUsername() {
        ...
        return (($model instanceof \M\MytUser) ? $model->getUser_name() : null);
    }
    ...
    public function getCurrentUser() {
        ...
        if($this->_member === null){
            $this->_member = \M\Mapper\MytUser::getInstance()->findById($mid);
        }
        return $this->_member;
    }
    ...
    public function login($username, $password, $remember, $secure = null) {
        ...
        $memberMapper = \M\Mapper\MytUser::getInstance();
        $memberModel = $memberMapper->findByMobile($username);
        if (!$memberModel instanceof \M\MytUser || \Ku\Tool::valid($password, $memberModel->getPassword(), $secure) === false) {
            return $this->getMsg(23203, false);
        }
        $lastTime = $memberModel->getLogin_at();
        $ip = \Ku\Tool::getClientIp();
        $memberModel->setLogin_ip($ip);
        $memberModel->setLogin_at(date('YmdHis'));
        $memberMapper->update($memberModel);
    ...
    public function checkRemberLogin() {
        ...
        $mapper = \M\Mapper\MytUser::getInstance();
        $model = $mapper->findById($memberId);
        if ($model instanceof \M\MytUser) {
...
//    }
    public function getLoginUser(){
        if(empty($this->_member)){
            $session = \Yaf\Session::getInstance();
            $id =  $session->get('mid');
            $this->_member = \M\Mapper\MytUser::getInstance()->findById($id);
            return $this->_member;
        }
        return $this->_member;
    }
